...
x=$(uname -m)
if [ "$x" = "x86_64" ]; then
  wget https://www.hls-proxy.com/downloads/8.4.8/hls-proxy-8.4.8.linux-x64.zip -O /var/tmp/hlsproxy.zip
else
  wget https://www.hls-proxy.com/downloads/8.4.8/hls-proxy-8.4.8.linux-arm64.zip -O /var/tmp/hlsproxy.zip
fi
